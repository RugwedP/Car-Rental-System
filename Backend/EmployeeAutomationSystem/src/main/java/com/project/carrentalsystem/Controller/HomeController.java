...
//	    }
	    ...
	    public String handleFirstLogin(@RequestParam Map<String, String> allParams, Model model) {
	        ...
	        try {
	            HttpEntity<Map<String, String>> request = new HttpEntity<>(passwordData);
	            ResponseEntity<Boolean> response = restTemplate.exchange(apiEndpoint, HttpMethod.PUT, request, Boolean.class);

	            if (response.getStatusCode().is2xxSuccessful()) {
	                boolean updateSuccessful = response.getBody() != null && response.getBody();

	                if (updateSuccessful) {
	                    // Fetch all employees if the password update is successful
	                    ...
	                    // Password was not strong or the update failed
	                    model.addAttribute("error", "New password must be at least 8 characters long, and include an uppercase letter, a lowercase letter, a number, and a special character.");
	                }
	            } else {
	                model.addAttribute("error", "Unexpected error occurred while updating the password.");
	            }
	        } catch (HttpClientErrorException | HttpServerErrorException e) {
	            model.addAttribute("error", "Failed to update the password. Please try again.");
	        }

	        return "loginPage"; // Redirect to login page if password update fails
	    }
